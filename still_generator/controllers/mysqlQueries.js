...
module.exports = {
  ...
  createTable : function(){
    ...
    connection.query('CREATE TABLE IF NOT EXISTS stills ( ' +
                    'still_id INT(11) NOT NULL AUTO_INCREMENT KEY, ' +
                    ...
                    'createdon TIMESTAMP)', function( err, result ){
                      ...
                      console.log( "Tabla 'stills' ha sido creada." );
  ...
  getLatest : function( callback ){
    ...
    connection.query( 'SELECT still_id FROM stills ORDER BY still_id DESC LIMIT 1', function( err, result ){
      if( err ) throw err;
      if( result[ 0 ] ){
        callback (result[ 0 ].still_id );
      }else{
        callback( 0 );
      }
  ...
  register : function( badge_id, filename, fileurl, md5Hash, callback ){
    connection.query( 'INSERT INTO stills ( ' +
